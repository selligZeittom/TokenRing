...
const osThreadAttr_t audio_attr = {
  .stack_size = 512,          	// Create the thread stack size of 256 bytes
	.priority = osPriorityAboveNormal, //Set initial thread priority to normal
};
...
*/
	...
	// create threads
	//------------------------------------------------------------------------------
//	audio_id = osThreadNew(AudioPlayer, NULL,  &audio_attr);    // Create thread
  debug_id = osThreadNew(DebugStation, NULL,  &debug_attr);    // Create thread
  phy_rec_id = osThreadNew(PhReceiver, NULL,  &phy_rec_attr);    // Create thread
  phy_snd_id = osThreadNew(PhSender, NULL,  &phy_snd_attr);    // Create thread
  mac_rec_id = osThreadNew(MacReceiver, NULL,  &mac_rec_attr);    // Create thread
  mac_snd_id = osThreadNew(MacSender, NULL,  &mac_snd_attr);    // Create thread
  time_rec_id = osThreadNew(TimeReceiver, NULL,  &time_rec_attr);    // Create thread
  time_snd_id = osThreadNew(TimeSender, NULL,  &time_snd_attr);    // Create thread
  chat_rec_id = osThreadNew(ChatReceiver, NULL,  &chat_rec_attr);    // Create thread
  chat_snd_id = osThreadNew(ChatSender, NULL,  &chat_snd_attr);    // Create thread
  touch_id = osThreadNew(Touch, NULL,  &touch_attr);    // Create thread
  lcd_id = osThreadNew(LCD, NULL,  &lcd_attr);    // Create thread
	//------------------------------------------------------------------------------
